...
function routes (fastify, options, done) {
  ...
  fastify.get('/', { onRequest: [fastify.authenticate] }, async (req, reply) => {
    await reply.code(200).view('shortener.hbs', { title: 'Shorten a url', scriptN: reply.cspNonce.script, styleN: reply.cspNonce.style })
  })
  ...
  fastify.get('/user-urls', { onRequest: [fastify.authenticate] }, async (req, reply) => {
    ...
    try {
      ...
      const hbsObj = rows.length > 0 ? { urls: rows, ...nonce } : { ...nonce }
  ...
  fastify.post('/shorten', shortenOpts, async (req, reply) => {
    ...
    try {
      ...
      if (rows.length > 0) {
        ...
        return reply.code(409).view('shortener.hbs', { title: 'You already shortened this url', shortenedUrl: `${rows[0].id}`, ...nonce })
      }
      const randomId = (Math.random() + 1).toString(36).substring(7)
      const inserted = await client.query(
        'INSERT INTO urls(id, url, user_id) VALUES($1, $2, $3) RETURNING *', [randomId, urlString, userId]
      )
      return reply.code(201).view('shortener.hbs', { title: 'Heres your shortened url', shortenedUrl: `${inserted.rows[0].id}`, ...nonce })
    } catch (e) {
      return reply.send(e)
    } finally {
      client.release()
